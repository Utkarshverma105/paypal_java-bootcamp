...
public class JavaStreamEx {
	...
	public static void main(String args[]) {
		...
		System.out.println("Squares List: " + squaresList); // 9, 4, 49, 25
		...
		count = strings
					.stream()	// create stream instance ["abc", "", "bc", "efg", "abcd", "", "jkl"]
//					.filter(s -> s.isEmpty()) // intermediate operation ["", ""]
		...
		count = strings
					.stream()
//					.filter(s -> s.length() == 3)
				    .filter(JavaStreamEx::checkLength)
					.count();
		...
		mergedString = strings
						.stream()
						.filter(string -> !string.isEmpty() && string.length() == 3)
						.collect(Collectors.joining(", "));
		System.out.println("Merged String: " + mergedString);

		squaresList = numbers
						.stream()			// [3, 2, 2, 3, 7, 3, 5]
						.distinct()			// [3, 2, 7, 5]
						.map(i -> i * i)	// [9, 4, 49,  25]
				        .sorted()
						.collect(Collectors.toList());
...
//		System.out.println("Empty Strings: " + count);
	...
	private static boolean checkLength(String item) {
	...
	private static String getMergedStringUsingJava7(List<String> strings, String separator) {
		...
		String mergedString = stringBuilder.toString();  // abc, bc, efg, abcd, jkl,
